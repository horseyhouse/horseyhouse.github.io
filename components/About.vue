<template>
  <h3>
    <img src="../media/about.gif" alt="About">
  </h3>
  <h5>Meet the hoez </h5>
  <ol>
    <li class="myli">
      <p class="myp">Theia</p>
      <img src="../photos/theia.jpg" alt="Theia" class="myimg">
        <h4>she/her</h4>
        <h5>dead bird collector</h5>
        <h5>phd baddie</h5>
        <h5>makes a mean ooey gooey sauce</h5>
    </li>
    <li class="myli">
      <p class="myp">Moogy</p>
      <img src="../photos/moog.jpg" alt="Moogy" class="myimg">
      <h4>she/they</h4>
      <h5>trash gremlin</h5>
      <h5>noodle wrangler</h5>
      <h5>general whore</h5>
    </li>
    <li class="myli">
      <p class="myp">Lydia</p>
      <img src="../photos/lyd.jpeg" alt="Lydia" class="myimg">
        <h4>she/her</h4>
        <h5>mommy</h5>
        <h5>will check you into the boards</h5>
        <h5>good with her hands ;)</h5>
    </li>
    <li class="myli">
      <p class="myp">Ru</p>
      <img src="../photos/ru.jpg" alt="Ru" class="myimg">
      <h4>they/them</h4>
      <h5>LOVES trains</h5>
      <h5>himbo appreciator</h5>
      <h5>crossword slut</h5>
    </li>
    <li class="myli">
      <p class="myp">Quinn</p>
      <img src="../photos/quinn.jpg" alt="Quinn" class="myimg">
      <h4>she/they</h4>
      <h5>resident hippie</h5>
      <h5>art hoe on that music shit!</h5>
      <h5>ayye yall wanna smoke a bowl</h5>
    </li>
    <li class="myli">
      <p class="myp">Noodle</p>
      <img src="../photos/noodle.jpg" alt="Noodle" class="myimg">
        <h4>she/they</h4>
        <h5>rodent on stilts</h5>
        <h5>snuggly baby</h5>
        <h5>poops on the floor</h5>
    </li>
    <li class="myli">
      <p class="myp">Jinni</p>
      <img src="../photos/jinni.jpg" alt="Jinni" class="myimg">
        <h4>she/her</h4>
        <h5>sweet n' sour old lady</h5>
        <h5>loves butt scratches</h5>
        <h5>poops in the closet</h5>
    </li>
    <li class="myli">
      <p class="myp">Sprinkles</p>
      <img src="../photos/sprink.jpg" alt="Sprinkles" class="myimg">
        <h4>she/they</h4>
        <h5>bright eyes, smooth brain</h5>
        <h5>shoelace connoisseur</h5>
        <h5>bee herder</h5>
    </li>
  </ol>

  <h5>Alumni</h5>
  <ol>
    <li class="myli">
      <p class="myp">Emily</p>
      <img src="../photos/emily.jpg" alt="Jinni" class="myimg">
        <h4>grem/lin</h4>
        <h5>future lawyer of america</h5>
        <h5>emo and tormented</h5>
        <h5>gamer girl weeb</h5>
    </li>
    <li class="myli">
      <p class="myp">Stinky</p>
      <img src="../photos/stinky.jpg" alt="Jinni" class="myimg">
        <h4>he/him</h4>
        <h5>explosive diarrhea</h5>
        <h5>noodle's bff</h5>
        <h5>humongous</h5>
    </li>
  </ol>
</template>
